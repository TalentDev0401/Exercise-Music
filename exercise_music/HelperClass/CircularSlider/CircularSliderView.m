...
static float barRadius, knobRadius, knobAngle, minValue, maxValue;
...
-(id)initWithMinValue:(float)minimumValue maxValue:(float)maximumValue initialValue:(float)initialValue
{
    ...
    if (self)
    {
        ...
        float percentDone = 1.0-(initialValue/(maxValue - minValue));
        
        knobAngle = MIN_ANGLE+(percentDone*(MAX_ANGLE-MIN_ANGLE));
        
        valueLabel = [[UILabel alloc] init];
        valueLabel.textAlignment = NSTextAlignmentCenter;
        [valueLabel setFrame:CGRectMake(100,300, 100, 40)];
      ...
      //  mpVolumeViewParentView.backgroundColor = [UIColor clearColor];
        ...
        [[MPVolumeView alloc] initWithFrame:CGRectMake(-100, -100, 0, 0)];
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (isKnobBeingTouched)
    {
        ...
        [valueLabel setText:[NSString stringWithFormat:@"Value: %.1f",[self value]]];
         ...
         [[MPMusicPlayerController iPodMusicPlayer] setVolume:[self value]];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
   ...
   [[NSNotificationCenter defaultCenter] postNotificationName:@"AnythingAtAllOne" object:nil userInfo:nil];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    CFArrayRef colors = (__bridge CFArrayRef) [NSArray arrayWithObjects:[UIColor whiteColor].CGColor, [UIColor whiteColor].CGColor, nil];
