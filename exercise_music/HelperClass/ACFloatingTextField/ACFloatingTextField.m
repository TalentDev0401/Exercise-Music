...
-(BOOL)isValidate
{
    //NSLog(@"Self.text == %@ && self.isMendatory == %i",self.text, self.isMendatory);
    if (self.isMendatory && [self.text isEqualToString:@""])
    {
        self.errorText = self.errorText;
        [self showError];
        return false;
    }
    if (![self checkForRegularExpression])
    {
        self.errorText = self.validateErrorText;
        [self showError];
        return false;
    }
    return true;
...
-(BOOL)checkForRegularExpression
{
    if (self.regEx != nil && self.text != nil)
    {
...
-(void)addRegEx:(NSString *)regEx Message:(NSString *)message
{
    self.regEx = regEx;
    self.validateErrorText = message;
}
@end
